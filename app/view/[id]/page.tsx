...
import { Copy, Check, Eye, Shield, AlertTriangle, Clock, Key } from "lucide-react";
		...
		try {
			...
			if (result.success && result.data) {
				...
				try {
					...
					setError(
						"Impossible de déchiffrer le contenu. La clé de chiffrement est peut-être incorrecte ou corrompue.",
					);
				}
			} else {
					...
					<Card>
						...
						<CardContent className="space-y-6">
							...
							<Alert>
								...
								<AlertDescription>
									<strong>Avis de sécurité :</strong> Ce contenu a été déchiffré localement dans votre
									navigateur. Le serveur n'a jamais eu accès à vos données non chiffrées ni à la clé
									de déchiffrement.
								</AlertDescription>
							</Alert>
							...
							<Alert>
								...
								<AlertDescription>
									<strong>Important :</strong> Ce contenu a été consulté et peut être automatiquement
									détruit selon les paramètres d'expiration. Sauvegardez-le en sécurité si nécessaire.
								</AlertDescription>
							</Alert>

							<div className="text-center">
								<Link href="/create">
									<Button>Créer un secret</Button>
								</Link>
							</div>
						</CardContent>
					</Card>
				...
				<Card>
					...
					<CardContent>
						...
						<form onSubmit={handleAccess} className="space-y-4">
							...
							{!encryptionKey && (
								<Alert variant="destructive">
									<AlertTriangle className="w-4 h-4" />
									<AlertDescription>
										Aucune clé de chiffrement trouvée dans l'URL. Assurez-vous d'utiliser le lien de
										partage complet, y compris la partie fragment (#).
										<br />
										<br />
						...
						<Alert className="mt-4">
							<Key className="w-4 h-4" />
							<AlertDescription>
								<strong>Zero-knowledge :</strong> Le déchiffrement se fait entièrement dans votre
								navigateur. Le serveur ne voit jamais votre clé de chiffrement ni le contenu déchiffré.
							</AlertDescription>
						</Alert>

						<div className="mt-6 text-center">
							<Link href="/create">
								<Button variant="outline">Créer un secret</Button>
							</Link>
						</div>
					</CardContent>
				</Card>
